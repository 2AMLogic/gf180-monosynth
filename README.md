...
| 5 | FPGA bitstream on real hardware | **routed, never programmed** — the joined chip (the REAL drum section) places and routes on an ECP5 LFE5U-25F at **57 % logic / 35 % FF / 50 % DSP**, post-route Fmax **30.49 MHz** against a derived 12.29 MHz constraint, bitstream written ([docs/fpga-build.md](docs/fpga-build.md)). The completed 808 (MODES 12→16, NUMS 6→11) was routed too and still fits at 59 % logic. The ULX3S wrapper now carries a real PLL — 25 MHz → **12.288135593 MHz, +11 ppm, +0.019 cents** ([docs/fpga-clock.md](docs/fpga-clock.md)); the previous build reported `EHXPLLL: 0/2`, ran the core at 25 MHz and would have played **1229.6 cents sharp**. iCE40 UP5K does **not** fit: 383 % logic, 175 % DSP. **Every FPGA figure this repo published before 2026-09-18 — 27 % logic, 43 % DSP, 33.01 MHz — was a chip with placeholder drums.** Nothing has been on a board and there is no host software |
