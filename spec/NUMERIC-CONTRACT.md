...
`model/test_modal_fixed.py`, 45 tests) lock the sizing decisions; the RTL
...
| `k` | 17 u | voice | ladder resonance, 4·res in Q3.14 | `LadderFx.regs` |
...
within them; the model clamps to them (`LadderFx.regs`), and at every extreme
of all three and of `g` every pre-saturation value in the ladder stays below
2^27, inside the datapath width of 11.4.
...
|---|---|---|
| `phase[k]`, `inc[k]`, `e[k]`, `r[k]` | 0 | `inc = 0` gives `c = 0` by 6.6.3 and `(e, r) = (0, 0)` by 6.6.1; `(e, r)` are recomputed at the first SET_INC |
...
7. **Register-width clamps in the host conversion** (5.5) — **resolved in
   ...
   fires, and why `a_inc` and `rate` clamp rather than widen, is in 5.5.
